...
function device(over: Partial<LocalData> = {}): LocalData {
  return {
    ...
    settings: { theme: "system", accent: "clay", spotifyEnabled: true, showTasks: true, menubarTimer: true, chime: false, chimeSound: "bell", updated_at: 0 },
    ...over,
  };
}
...
describe("syncOnce orchestration", () => {
  ...
  it("settings follow last-write-wins", async () => {
    const server = new FakeServer();
    let a = await run(server, device({ settings: { theme: "dark", accent: "forest", spotifyEnabled: false, showTasks: true, menubarTimer: true, chime: false, chimeSound: "bell", updated_at: 10 } }), emptySyncState(), "A");
    await run(server, device({ settings: { theme: "light", accent: "red", spotifyEnabled: true, showTasks: true, menubarTimer: true, chime: false, chimeSound: "bell", updated_at: 20 } }), emptySyncState(), "B");
    a = await run(server, a.local, a.state, "A");
